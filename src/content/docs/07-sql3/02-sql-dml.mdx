...
Si ajoute des valeurs pour toutes les colonnes, on peut omettre les noms des colonnes :
...
On peut tout à fait insérer des données depuis une autre table en utilisant une requête `SELECT` :
...
On peut tout à fait supprimer des données depuis une autre table en utilisant une requête `SELECT` :
...
1. Ajouter un nouveau client dans la table `clients` de la base de données `Boutique` avec les informations suivantes : numero : `10` , nom : `Dupont` , prenom : `Jean` , rue : `6 Rue de la Paix` , ville : `Paris` , code_postal : `75002` , telephone : `01 23 45 67 89` , categorie : `C2` , compte : `1000.0`

<details>
<summary>Solution</summary>

```sql
INSERT INTO clients (numero, nom, prenom, rue, ville, code_postal, telephone, categorie, compte)
