...
11. On suppose que le concepteur de la base de données n’a pas trouvé utile d’imposer un identifiant primaire sur la table `Produits`
    ...
    (a) Ecrire une requête qui recherche les valeurs de NPRO présentes en plus d’un exemplaire.

    (b) Ecrire une requête qui indique combien de valeurs de NPRO sont présentes en plus d’un exemplaire.
