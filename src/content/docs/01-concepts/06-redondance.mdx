...
On pourra écrire cette relation 

``` R = <{Prosesseur, Heure, Salle, Classe, Matiere}, {P→M, HC→S, PH→C, HS→P}> ```
...
* On dit que ISBN est le **déterminant** et (TITRE, AUTEUR) les **déterminés** 
...
* On semble pouvoir déduire que `ISBN, DATE_ACHAT → TITRE, AUTEUR, RAYONNAGE`
