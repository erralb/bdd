...
- Soit proposer le code SQL pour une procédure stockée dans un autre SGBD (MySQL, PostgreSQL, SQL Server, Oracle, etc.)
    - Par exemple, vous pouvez directement importer l'[export de la clinique non normalisée](bdd/public/clinique/clinique-non-normalisee-EXPORT.sql) dans [MySQL](https://dev.mysql.com/downloads/installer/), puis executer vos requêtes DDL pour créer les tables normalisées
