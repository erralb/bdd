...
```sql
SELECT nom, prenom, telephone FROM Clients WHERE telephone IS NULL;
```
...
```sql
SELECT  nom, prenom, telephone FROM Clients WHERE telephone IS NOT NULL;
```
...
```sql
SELECT  nom, prenom, ville FROM Clients WHERE ville IN ('Leclercq', 'Jacquot');
```
...
```sql
SELECT  nom, prenom, ville FROM Clients WHERE ville NOT IN ('Leclercq', 'Jacquot');
```
...
```sql
SELECT * FROM Commandes WHERE dateCommande BETWEEN '2024-01-01' AND '2024-04-01';
```
...
```sql
SELECT * FROM Commandes WHERE dateCommande NOT BETWEEN '2024-01-01' AND '2024-04-01';
```
...
```sql
SELECT * FROM Clients WHERE ville LIKE '_eclercq';
```
...
```sql
SELECT * FROM Clients WHERE ville LIKE '%cq%';
```
...
```sql
SELECT * FROM Clients WHERE ville = 'Leclercq' AND categorie = 'C3';
```
<Table
  sqliteDB="public/sample/ecommerce/ecommerce-fr-erd-v2.sqlite"
  title="Clients"
...
* Les conditions de sélection sont définies dans la clause `WHERE`
* Les conditions de sélection sont des expressions booléennes
* Les conditions de sélection peuvent être composées d'opérateurs de comparaison logiques
