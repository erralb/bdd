 ...
 * 1. En utilisant une jointure (vue plus tard dans le cours)
 * 2. En utilisant une sous-requête
...
Supposons que l'on veuille obtenir les commandes des clients habitant `Jacquot`

1. On peut retrouver les numéros des clients habitant `Jacquot` en exécutant la requête :

```sql
SELECT numero FROM Clients WHERE ville = 'Jacquot' ;
```
...
2. Il est alors facile de retrouver les commandes des clients habitant à `Jacquot` :
...
Cette procédure n'est pas pratique... surtout si de nouveaux clients de `Jacquot` s'ajoutent à la base de données
